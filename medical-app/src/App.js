...
async function runGeminiAnalysis(ocrText) {
  const prompt = `You are a medical AI assistant. Analyze this medical report and return ONLY valid JSON with no markdown, no explanation, no backticks.
  ...
  "abnormalValues": [
    {
      ...
      "status": "HIGH or LOW or BORDERLINE or NORMAL",
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ocr-card-title { font-family:'Nunito'; font-size:16px; font-weight:800; color:#111827; margin-bottom:4px; }
  ...
  .term-chip { display:inline-flex; align-items:center; gap:6px; background:#F5F3FF; border:1px solid #DDD6FE; border-radius:10px; padding:8px 12px; margin:4px; cursor:pointer; }
  ...
  .risk-meter-track { height:12px; background:#F3F4F6; border-radius:6px; overflow:hidden; margin:8px 0; }
  ...
  .risk-item-desc { font-size:11px; color:#6B7280; line-height:1.5; }
  .ai-loading { display:flex; flex-direction:column; align-items:center; gap:12px; padding:32px 0; }
  .ai-spinner { width:48px; height:48px; border:4px solid #EDE9FE; border-top:4px solid #7C3AED; border-radius:50%; animation:spin .8s linear infinite; }
...
function Register({ onBack }) {
  ...
  return (
    <div className="screen auth-screen">
      <div className="status-bar"><span>9:41</span><span>🔋 WiFi</span></div>
      <div className="auth-header"><button className="back-btn" onClick={onBack}>←</button><span style={{fontSize:15,fontWeight:700,color:"#111827"}}>Create Account</span></div>
...
function Login({ onBack, onRegister }) {
  ...
  return (
    <div className="screen auth-screen">
      <div className="status-bar"><span>9:41</span><span>🔋 WiFi</span></div>
      <div className="auth-header"><button className="back-btn" onClick={onBack}>←</button><span style={{fontSize:15,fontWeight:700,color:"#111827"}}>Sign In</span></div>
      <div style={{padding:"0 20px 20px",flex:1}}>
        <div className="auth-card">
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:16}}>
      <div className="page-header">
        <div><div style={{fontSize:13,color:"#6B7280"}}>Welcome back 👋</div><div className="page-title">{user.displayName||"User"}</div></div>
      ...
      <div style={{margin:"4px 20px 0",background:"linear-gradient(135deg,#F5F3FF,#EDE9FE)",borderRadius:16,padding:16,border:"1px solid #DDD6FE"}}>
        <div style={{fontSize:13,fontWeight:800,color:"#7C3AED",marginBottom:4}}>🤖 AI Analysis — Powered by Gemini</div>
        <div style={{fontSize:12,color:"#6B7280",lineHeight:1.5}}>Abnormal value detection, medical term explanations, and risk prediction — powered by Google Gemini AI.</div>
      </div>
    </div>
  );
}
...
function UploadTab({ user, onFileUploaded }) {
  ...
  const handleFile = (f) => {
    ...
    const err = validate(f); if (err) { setError(err); return; } setFile(f);
  };
  const doUpload = () => {
    if (!file) return;
    ...
    const iv=setInterval(()=>{
      ...
      if(p>=100){p=100;clearInterval(iv);
        setTimeout(()=>{
          addDoc(collection(db,"reports"),{userId:user.uid,userName:user.displayName||user.email,fileName:file.name,fileSize:(file.size/1024/1024).toFixed(2)+" MB",fileType:file.type,uploadedAt:serverTimestamp(),status:"uploaded"}).catch(()=>{});
          setUploading(false); setUploaded(true); onFileUploaded(file);
        },300); return;
      }
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:16}}>
      <div className="page-header"><div><div className="page-title">Upload Report</div><div className="page-sub">US-02 · PDF, JPG, PNG — max 10MB</div></div></div>
      ...
      <div className={`drop-zone ${dragging?"drag":""} ${file&&!error?"has-file":""}`}
        ...
        {uploading&&(<><div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#6B7280",width:"100%",marginBottom:4}}><span>Uploading...</span><span>{progress}%</span></div><div className="prog-bar"><div className="prog-fill" style={{width:`${progress}%`}}/></div></>)}
      ...
      {file&&!error&&(<div className="file-info"><div style={{fontSize:28}}>{ext==="PDF"?"📄":"🖼️"}</div><div style={{flex:1}}><div className="file-name">{file.name}</div><div className="file-size">{sizeMB} MB • {ext}</div></div><span className={`badge ${uploaded?"b-green":"b-blue"}`}>{uploaded?"✓ Done":"Ready"}</span></div>)}
      <div style={{padding:"0 20px"}}>
        {file&&!error&&!uploaded&&<button className="btn-blue" onClick={doUpload} disabled={uploading}>{uploading?"⏳ Uploading...":"📤 Upload Report →"}</button>}
        {file&&!error&&<button className="btn-red" onClick={()=>{setFile(null);setUploaded(false);setProgress(0);setError("");}}>🗑️ Clear File</button>}
      </div>
    </div>
  );
}

function OcrTab({ uploadedFile, onOcrDone, onGoToAI }) {
  ...
  const runOcr = () => {
    ...
    const iv=setInterval(()=>{
      ...
      if(p>=100){
        ...
        setConfidence(conf); setText(extractedText); setStage("done");
        onOcrDone(extractedText);
        addDoc(collection(db,"ocr_results"),{fileName:uploadedFile.name,confidence:conf,extractedText,processedAt:serverTimestamp()}).catch(()=>{});
        return;
      }
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:16}}>
      <div className="page-header"><div><div className="page-title">OCR Extraction</div><div className="page-sub">US-03 · AI-powered text extraction</div></div></div>
      {!uploadedFile&&<div style={{margin:"0 20px"}}><div className="info-box">ℹ️ Upload a report first in the Upload tab.</div></div>}
      {uploadedFile&&(
              ...
              <div className="step-list">
                {steps.map((s,i)=>(
                  <div key={i} className="step-item">
                    ...
                    <div><div className={`step-label ${i<stepIdx?"done":""}`}>{s.label}</div>{i===stepIdx&&<div className="step-sublabel">↺ {s.sub}</div>}</div>
                  </div>
                ))}
              </div>
            </>
          ...
          {stage==="done"&&(
            <>
              ...
              <div style={{marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#6B7280",marginBottom:4}}><span>Confidence</span><span style={{fontWeight:700,color:confColor}}>{confidence}%</span></div>
              ...
              <button className="btn-purple" onClick={()=>onGoToAI(text)}>🤖 Go to AI Analysis →</button>
...
function AiTab({ ocrText }) {
  ...
  const runAnalysis = async () => {
    ...
    try {
      const data = await runGeminiAnalysis(ocrText);
      setResult(data);
      setStage("done");
    } catch(e) {
      console.error(e);
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:16}}>
      <div className="page-header">
        <div><div className="page-title">AI Analysis</div><div className="page-sub">US-04 · Powered by Google Gemini</div></div>
        <span className="badge b-purple">🤖 AI</span>
      </div>

      {!ocrText&&<div style={{margin:"0 20px"}}><div className="info-box">ℹ️ Complete OCR extraction first, then come back here.</div></div>}

      {ocrText&&stage==="idle"&&(
        <div className="ai-section">
          <div className="ai-section-title">🧠 Ready to Analyze</div>
          <div style={{fontSize:13,color:"#6B7280",marginBottom:16,lineHeight:1.6}}>Gemini AI will analyze your medical report for:</div>
          {[["🔴","Abnormal Value Detection","Flags out-of-range lab values"],["📚","Medical Term Identification","Explains complex terms in plain English"],["⚠️","Risk Prediction","Estimates health risks based on findings"]].map(([icon,title,desc])=>(
            <div key={title} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:"1px solid #F3F4F6"}}>
              <span style={{fontSize:20}}>{icon}</span>
              <div><div style={{fontSize:13,fontWeight:700,color:"#111827"}}>{title}</div><div style={{fontSize:11,color:"#9CA3AF",marginTop:2}}>{desc}</div></div>
            </div>
          ))}
      ...
      {stage==="loading"&&(
        <div className="ai-section">
          <div className="ai-loading">
            ...
            <div style={{fontSize:14,fontWeight:700,color:"#7C3AED"}}>Gemini AI is analyzing...</div>
      ...
      {stage==="error"&&(
        <div style={{margin:"0 20px"}}>
          ...
          <button className="btn-purple" onClick={runAnalysis}>↺ Try Again</button>
        </div>
      )}
      ...
      {stage==="done"&&result&&(
        <>
          ...
          <div className="ai-section">
            ...
            <div style={{fontSize:13,color:"#374151",lineHeight:1.7,background:"#F9FAFB",borderRadius:10,padding:12}}>{result.summary}</div>
                ...
                <div key={i} className="abnormal-item">
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      <span className="badge" style={{background:statusColor(v.status)==="#DC2626"?"#FEF2F2":statusColor(v.status)==="#7C3AED"?"#F5F3FF":"#FFFBEB",color:statusColor(v.status),fontSize:10}}>{v.status}</span>
                    </div>
                    <div style={{fontSize:12,fontWeight:700,color:statusColor(v.status),marginTop:2}}>{v.value}</div>
              ...
              {result.riskPredictions.map((r,i)=>(
                <div key={i} className="risk-item">
                  <div className="risk-item-header">
                    ...
                    <span className="badge" style={{background:r.riskLabel==="High"?"#FEF2F2":r.riskLabel==="Moderate"?"#FFFBEB":"#ECFDF5",color:riskColor(r.riskLabel)}}>{r.riskLabel} · {r.riskLevel}%</span>
                  </div>
                  <div className="risk-meter-track"><div className="risk-meter-fill" style={{width:`${r.riskLevel}%`,background:riskBg(r.riskLabel)}}/></div>
                  <div className="risk-item-desc">{r.reasoning}</div>
                </div>
              ))}
              <div className="disclaimer">⚠️ AI-generated estimates only. Consult a qualified physician for diagnosis.</div>
            </div>
          )}
...
function HistoryTab({ user }) {
  ...
  return (
    <div style={{flex:1,overflowY:"auto",paddingBottom:16}}>
      <div className="page-header"><div><div className="page-title">Upload History</div><div className="page-sub">All your uploaded reports</div></div></div>
      {loading&&<div style={{display:"flex",justifyContent:"center",padding:40}}><div className="spinner"/></div>}
      {!loading&&reports.length===0&&<div style={{margin:"0 20px"}}><div className="info-box">📭 No reports yet. Go to Upload tab to add one.</div></div>}
      {!loading&&reports.map(r=>(
        <div key={r.id} style={{margin:"0 20px 12px",background:"white",borderRadius:16,padding:16,border:"1px solid #E5E7EB",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:42,height:42,borderRadius:12,background:"#EFF6FF",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{r.fileType?.includes("image")?"🖼️":"📄"}</div>
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#111827",wordBreak:"break-all"}}>{r.fileName}</div><div style={{fontSize:12,color:"#9CA3AF",marginTop:2}}>{r.fileSize} • {r.fileType?.split("/")[1]?.toUpperCase()}</div></div>
            <span className="badge b-green">✓ Saved</span>
          </div>
          <div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #F3F4F6",fontSize:12,color:"#9CA3AF"}}>📅 {r.uploadedAt?.toDate?.()?.toLocaleString()||"Just now"}</div>
        </div>
      ))}
    </div>
  );
...
function Dashboard({ user }) {
  ...
  const handleGoToAI = (text) => { setOcrText(text); setTab("ai"); };
  const handleLogout = async () => { try { await signOut(auth); } catch(e) { console.log(e); } };
  ...
  return (
    <div className="screen" style={{background:"#F9FAFB"}}>
      ...
      {tab==="ocr"     && <OcrTab uploadedFile={uploadedFile} onOcrDone={setOcrText} onGoToAI={handleGoToAI} />}
...
export default function App() {
  ...
  if(loading) return (<><style>{css}</style><div style={{minHeight:"100vh",background:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center"}}><div className="shell"><div className="loading-screen"><div className="spinner"/><div style={{fontSize:14,fontWeight:700,color:"#6B7280"}}>Loading...</div></div></div></div></>);
  return (
    <><style>{css}</style>
